...
def logger(eth, ip, protocol, data, filename, counter):
    ...
    log.write(f'     - Flags\t\t\t\t: {split_flags_fragment.bin[:3]} ({hex(int(split_flags_fragment.bin[:3], 2))})\n')
    log.write(f'     - Fragment Offset\t\t: {split_flags_fragment.bin[3:]} ({int(split_flags_fragment.bin[3:], 2)})\n')
    log.write(f'     - Time-to-Live (TTL)\t: {ip[5]}\n')
    log.write(f'     - Protocol\t\t\t: {ip[6]}\n')
    log.write(f'     - Header Checksum\t\t: {hex(ip[7])}\n')
    ...
    if ip[6] == 6:
        ...
        log.write(f'     - Flags\t\t\t\t: {split_HL_flags[4:] + split_flags[0:]}\n')
        ...
        log.write(f'     - Checksum\t\t\t: {hex(protocol[7])}\n')
        ...
        log.write(f'     - Checksum\t\t\t: {hex(protocol[3])}\n\n')
    elif ip[6] == 1:
