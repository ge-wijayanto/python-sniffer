...
def getTCPHeader(tcp):
    ...
    split_version_TCP = BitArray(hex(tcp[4]))
    split_version_flags = BitArray(hex(tcp[5]))
    print(f'     - Header Length\t\t: {Fore.GREEN}{split_version_TCP.bin[:4]} ({int(split_version_TCP.bin[:4], 2)*4} bytes ({int(split_version_TCP.bin[:4], 2)}){Style.RESET_ALL}')
    print(f'     - Flags\t\t\t: {Fore.GREEN}{tcp[5]}{Style.RESET_ALL}')
    print(f'          Reserved\t: {Fore.GREEN}{split_version_TCP.bin[4:7]}{Style.RESET_ALL}')
    print(f'          Accurate ECN\t: {Fore.GREEN}{split_version_TCP.bin[7]}{Style.RESET_ALL}')
    print(f'          Congestion Window\t: {Fore.GREEN}{split_version_flags.bin[0]}{Style.RESET_ALL}')
    print(f'          ECN-Echo\t\t: {Fore.GREEN}{split_version_flags.bin[1]}{Style.RESET_ALL}')
    print(f'          URG: %d, ACK: %d, PSH: %d' % ({Fore.GREEN}(split_version_flags.bin[2], split_version_flags.bin[3], split_version_flags.bin[4])) + '{Style.RESET_ALL}')
    print(f'          - ACK\t\t\t: {Fore.GREEN}{split_version_flags.bin[3]}{Style.RESET_ALL}')
    print(f'          - PSH\t\t\t: {Fore.GREEN}{split_version_flags.bin[4]}{Style.RESET_ALL}')
    print(f'          - RST\t\t\t: {Fore.GREEN}{split_version_flags.bin[5]}{Style.RESET_ALL}')
    print(f'          - SYN\t\t\t: {Fore.GREEN}{split_version_flags.bin[6]}{Style.RESET_ALL}')
    print(f'          - FIN\t\t\t: {Fore.GREEN}{split_version_flags.bin[7]}{Style.RESET_ALL}')
