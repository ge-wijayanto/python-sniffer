...
def getIPHeader(ip):
    ...
    print(f'     - Flags\t\t\t: {Fore.GREEN}{split_flags_fragment.bin[:3]} ({int(split_flags_fragment.bin[:3], 2)}){Style.RESET_ALL}')
...
def getTCPHeader(tcp):
    ...
    print(f'     - Flags\t\t\t: {Fore.GREEN}{int(split_HL_flags[4:] + split_flags[0:7], 2)}{Style.RESET_ALL}')
